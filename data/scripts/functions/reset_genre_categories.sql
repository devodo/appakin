...
INSERT INTO category_app(category_id, app_id, "position", date_created)
  select gc.category_id, a.app_id,
    row_number() over(partition by gc.category_id order by coalesce(ar.ranking, 0) desc, a.release_date desc),
  ...
  and (
    ...
    (gc.primary_genre is null AND gc.genres <@ a.genres) OR
    (gc.primary_genre is null AND gc.genres is null AND 'Games' != ALL(a.genres))
  );
